...
public class PageIndex {
	public static String pageList(int page,int totpage,String url,String addtag) {
		int idx_pre, idx_start; //현재페이지--총페이지--url-- 
		...
		idx_pre = ((page-1) / 10);              // Priview 페이지 계산
	  	...
	  	if(idx_pre > 0) {
	  		s_pre = "<a href='"+url+"?page="+(idx_pre*10)+addtag+"'>"
	  				+ "\n <img src=./img/i_prev.gif width=20 height=10></a>";
	  	} else {
	  		s_pre = "\n <img src=./img/i_prev.gif width=20 height=10> ";
	  	}
		...
		// Next 표시부분
	  	if(idx_start <= totpage ) {
	  		s_next = "<a href='"+url+"?page="+idx_start+addtag+"'>"
	  				+ "\n <img src=./img/i_next.gif width=20 height=10></a>";
	  	} else {
	  		s_next = "\n <img src=./img/i_next.gif width=20 height=10>";
	  	}
		...
		idx_pre = ((page-1) / 10);              // Priview 페이지 계산
	  	...
	  	if(idx_pre > 0) {
	  		s_pre = "<a href='"+url+"?page="+(idx_pre*10)+"&search="+query+"&key="+ URLEncoder.encode(key)+"'>"
	  				+ "\n <img src=Board/img/i_prev.gif width=20 height=10></a>";
	  	} else {
	  		s_pre = "\n <img src=Board/img/i_prev.gif width=20 height=10> ";
	  	}
		...
		// Next 표시부분
	  	if(idx_start <= totpage ) {
	  		s_next = "<a href='"+url+"?page="+idx_start+ "&search="+query+"&key="+ URLEncoder.encode(key)+"'>"
	  				+ "\n <img src=Board/img/i_next.gif width=20 height=10></a>";
	  	} else {
	  		s_next = "\n <img src=Board/img/i_next.gif width=20 height=10>";
	  	}
