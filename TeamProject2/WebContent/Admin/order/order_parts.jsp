...
<div class="contain">
	<div class="sub-topcontent">
		<h2 class="sub-title">발주등록 - 부품</h2>
		<div class="sub-search">
			<form name="my" method="post" action="#" onsubmit="return check()">
				<select name="search" class="sel">
					<option value="p_name"<c:if test="${search=='p_name'}"> selected </c:if>>제품명</option>
					<option value="company"<c:if test="${search=='company'}"> selected </c:if>>회사명</option>
					<option value="p_model"<c:if test="${search=='p_model'}"> selected </c:if>>모델</option>
				</select>
...
<c:if test="${!empty plist}">
	<c:forEach var="porder" items="${plist}">					
				<tr>
					<td><input type="checkbox" name="chk"></td>
					<td>${porder.company}</td>
					<td>${porder.p_name}</td>
					<td>${porder.p_model}</td>
					<td>${porder.price}</td>
					<td>${porder.parts}</td>
					<td>O</td>
				</tr>
